#' @title Descriptive recovery metrics for a single disturbance
#'
...
#' @section Why `k_recovery` and `t_half` are often `NA`:
...
#' `"insufficient_positive_deficits"` and both `k_recovery` and `t_half` are
#' returned as `NA` rather than being fitted to two points.
#'
#' This is missingness by design, not failure. A rate estimated from a handful
#' of points spanning less than one recovery time constant is not informative,
#' and reporting it would invite over-interpretation. Always read `k_recovery`
#' together with `fit_status`, `n_fit` and `fit_r_squared`.
#'
#' If most trajectories return `NA`, extend the observation window rather than
#' lowering `min_pts`: as a rule of thumb the record should continue for at
#' least two or three times the expected recovery time after `perturb_end`.
#'
#' @examples
